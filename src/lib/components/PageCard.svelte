	...
	<div class="flex items-start justify-between gap-4 mb-4">
		<div class="flex items-center gap-3 flex-1 min-w-0">
			...
			<div class="min-w-0 flex-1">
				<h3 class="font-semibold text-base leading-tight group-hover:text-primary transition-colors truncate">
					{page.title}
				</h3>
				{#if page.path}
					<p class="text-xs text-muted-foreground font-mono truncate">{page.path}</p>
				{/if}
			</div>
		</div>
		<div class="opacity-0 group-hover:opacity-100 transition-opacity duration-200 shrink-0">
			<Button size="sm" variant="ghost" class="h-8 w-8 p-0 rounded-lg">
