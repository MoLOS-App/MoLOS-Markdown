<script lang="ts">
	...
	import { Edit, FolderOpen, X, Plus, Calendar, Hash, FileText as FileTextIcon } from "lucide-svelte";

	interface Props {
		page: MarkdownPage;
		existingPaths: string[];
		onUpdate: (data: { title?: string; tags?: string[]; path?: string }) => void;
		onClose: () => void;
	}

	let { page, existingPaths, onUpdate, onClose }: Props = $props();
	...
	// Extract path prefix from existing path (remove slug)
	$effect(() => {
		if (page.path) {
			const parts = page.path.split("/").filter(Boolean);
			if (parts.length > 1) {
				formData.pathPrefix = "/" + parts.slice(0, -1).join("/");
			} else {
				formData.pathPrefix = "/";
			}
		} else {
	...
	<Dialog.Content class="max-w-[90vw] sm:max-w-md">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-3">
				<div class="p-2 bg-blue-500/10 rounded-lg">
					<Edit class="h-5 w-5 text-blue-500" />
				</div>
				<div>
					Edit Page
					<p class="text-xs sm:text-sm font-normal text-muted-foreground mt-0.5">Update page details below</p>
				</div>
			</Dialog.Title>
		</Dialog.Header>
		...
		<div class="space-y-4 py-4">
			...
			<div class="pt-2 border-t border-border/50">
				<Label class="text-xs font-medium text-muted-foreground mb-2 block">Page Info</Label>
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
					...
					<div class="flex items-center gap-1.5 text-muted-foreground">
						...
						<span>{page.content?.split(/\s+/).filter(x => x).length || 0} words</span>
					</div>
					<div class="flex items-center gap-1.5 text-muted-foreground">
						<Calendar class="h-3.5 w-3.5" />
						<span>Created {new Date(page.createdAt * 1000).toLocaleDateString()}</span>
					</div>
					<div class="flex items-center gap-1.5 text-muted-foreground">
						<Calendar class="h-3.5 w-3.5" />
						<span>Updated {new Date(page.updatedAt * 1000).toLocaleDateString()}</span>
					</div>
				</div>
			</div>
		</div>
